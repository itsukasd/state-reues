...
class TrieNode:
    def __init__(self, index=-1, context=None, children=None):
        self.index = index
...
def getTotalTime(beginFlag):
    f=open('D:\preixTree\log.txt','r')
    totalTime=0
    lines=f.readlines()
    for i in range(len(lines)):
        if re.match(beginFlag, lines[i]):
            lines=lines[i+1:]
            break
    for line in lines:
        tmobj = re.match(r'@@@(.*)\$\$\$',line)
        if tmobj:
            tm=tmobj.group(1)
            totalTime=totalTime+eval(tm)
    log('# totalTime=%s' % totalTime)
    return totalTime
...
if __name__=="__main__":
    #start
    # log('\n\n')
    # begin='#####begin#####'+datetime.now().strftime("%Y-%m-%d, %H:%M:%S")
    # log(begin)

    #workplace
    testcase_in_dir=r'D:\preixTree\test_srcipt\wfh'
    work_path = create_work_path(testcase_in_dir)
    extract_path=os.path.join(work_path,'_extract_')
    single_path=os.path.join(work_path,'_single_')
    log("# workplace established")
    ...
    # log("# Single file test over")
    # # normal=getTotalTime(begin)
    ...
    time3=time.time()
    log("# All the files have inserted into the tree")
    log(time3-time2)
    ...
    # #calculate
    # alltime = getTotalTime(begin)
    # if alltime != 0:
    #     log("preTime = %s" % normal)
    #     log("afterTime = %s" % (alltime-normal))
    #     log("accRate = %s" % (normal/(alltime-normal)))
